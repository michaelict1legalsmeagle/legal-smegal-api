...
# ── S33-STEP4b — Condition override (user-set, NEVER read by the comp/ ──────
# ceiling pipeline). See ceiling_engine.py module docstring for the boundary
# this enforces: this is evidence a HUMAN looks at to inform their OWN
# judgement about whether nearby comps reflect this property's likely
# condition. The system never derives a price adjustment from this field or
# from any uploaded photo. Mirrors the _resolved_flags persistence pattern
# (optimistic lock) exactly, for consistency with the rest of the platform.
#
# Valid "assessment" values: "comparable" | "worse_than_comps" | "unknown"
# These are deliberately NOT named "light/moderate/full" or any vocabulary
# from the Financial Model's refurb-tier selector — different question
# (is this evidence fair to compare against?), different field, one-way only.
_CONDITION_ASSESSMENT_VALUES = {"comparable", "worse_than_comps", "unknown"}

@app.route("/api/deals/<deal_id>/condition-override", methods=["POST", "OPTIONS"])
@require_auth
def save_condition_override(deal_id: str):
    """Persist the user's own condition assessment to
    summary_json._condition_override. Body:
      { assessment: "comparable"|"worse_than_comps"|"unknown",
        note: "free text the user wrote, optional",
        photo_ids: ["<storage object id>", ...]  (optional, set via
          /api/deals/<deal_id>/condition-photos/upload below) }
    This field is display-only context for the user. It is NEVER read by
    ceiling_engine.py or any comp-weighting code — see that module's
    docstring for the explicit boundary."""
    if request.method == "OPTIONS":
        return jsonify({}), 200
    if not supabase:
        return jsonify({"error": "Database unavailable"}), 503
    try:
        ...
        if assessment not in _CONDITION_ASSESSMENT_VALUES:
            return jsonify({
                "error": f"assessment must be one of {sorted(_CONDITION_ASSESSMENT_VALUES)}"
            }), 400
        note = (body.get("note") or "").strip()[:2000]  # generous cap, not unbounded
        ...
        if not result.data:
            ...
            if row2.data:
                sj2 = dict(row2.data.get("summary_json") or {})
                sj2["_condition_override"] = sj["_condition_override"]
                supabase.table("deals").update({
                    "summary_json": sj2, "updated_at": now_iso(),
                }).eq("id", deal_id).eq("user_id", request.user_id) \
                  .eq("updated_at", row2.data.get("updated_at")).execute()

        return jsonify({"ok": True, "condition_override": sj["_condition_override"]}), 200
    except Exception as e:
        app.logger.exception("save_condition_override failed")
        app.logger.error("Unhandled exception: %s", e, exc_info=True); return jsonify({"error": "An internal error occurred"}), 500


@app.route("/api/deals/<deal_id>/condition-override", methods=["GET"])
@require_auth
def get_condition_override(deal_id: str):
    """Return the user's persisted condition assessment, if any."""
    if not supabase:
        return jsonify({"error": "Database unavailable"}), 503
    try:
        row = supabase.table("deals").select("summary_json") \
        ...
        if not row.data:
            return jsonify({"error": "Deal not found"}), 404
        sj = row.data.get("summary_json") or {}
        return jsonify({"ok": True, "condition_override": sj.get("_condition_override") or None}), 200
    except Exception as e:
        app.logger.error("Unhandled exception: %s", e, exc_info=True); return jsonify({"error": "An internal error occurred"}), 500


_ALLOWED_PHOTO_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp", ".heic"}
_ALLOWED_PHOTO_MAGIC_BYTES = (
    (b"\xff\xd8\xff", "jpg"),
    (b"\x89PNG\r\n\x1a\n", "png"),
    (b"RIFF", "webp"),  # webp is RIFF container — broad check, refined below
)

@app.route("/api/deals/<deal_id>/condition-photos/upload", methods=["POST", "OPTIONS"])
@require_auth
def upload_condition_photo(deal_id: str):
    """Upload a photo as supporting CONTEXT for the user's own condition
    assessment (see save_condition_override above). The photo is stored and
    displayed back to the user alongside their override — it is NEVER
    analysed by any model, NEVER converted into a number, and NEVER read by
    ceiling_engine.py. This is deliberate: an unaudited, uncalibrated
    image-derived "condition score" sitting in the same pipeline as RICS-
    grounded comp data and HMLR's own PPD category fields would undermine
    the traceability standard the rest of this platform holds to. The photo
    exists purely so the user has the image in front of them while they set
    their own assessment — same evidentiary role as a human valuer's site
    visit photo, not a model input.
    Multipart form: file (image), deal_id (required).
    Returns: { photo_id, storage_path }"""
    if request.method == "OPTIONS":
        return jsonify({}), 200
    if not supabase:
        return jsonify({"error": "Database unavailable"}), 503
    ...
    if ext not in _ALLOWED_PHOTO_EXTENSIONS:
        return jsonify({"error": f"Unsupported file type. Allowed: {sorted(_ALLOWED_PHOTO_EXTENSIONS)}"}), 400

    try:
        deal_check = supabase.table("deals").select("id") \
            .eq("id", deal_id).eq("user_id", request.user_id).single().execute()
    ...
    if len(file_bytes) > 10 * 1024 * 1024:  # 10MB per photo — generous for a phone photo, bounded
        return jsonify({"error": "Photo exceeds 10MB limit"}), 413

    # Magic-byte check, same defensive posture as the PDF upload path.
    if not any(file_bytes.startswith(sig) for sig, _ in _ALLOWED_PHOTO_MAGIC_BYTES):
        return jsonify({"error": "File content does not match an accepted image format"}), 400

    photo_id = str(uuid.uuid4())
    storage_path = f"{request.user_id}/{deal_id}/{photo_id}{ext}"
    try:
        supabase.storage.from_("condition-photos").upload(
            storage_path, file_bytes,
            {"content-type": file.mimetype or "image/jpeg"}
        )
    except Exception as e:
        app.logger.exception("condition photo upload failed")
        return jsonify({"error": "Photo storage failed"}), 502

    return jsonify({"ok": True, "photo_id": photo_id, "storage_path": storage_path}), 200
