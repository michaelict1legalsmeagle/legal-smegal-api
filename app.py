    ...
    def generate():
        ...
        try:
            ...
            try:
                ...
                # Stage 1 — extraction (run in thread, heartbeat while waiting)
                ...
                # Run LLM in background thread, yield heartbeats to keep SSE alive
                _result = {}
                def _run_stage1():
                    try:
                        _result["data"] = llm_json_raw(
                ...
                t1 = _threading.Thread(target=_run_stage1, daemon=True)
                t1.start()
                elapsed = 0
                while t1.is_alive():
                    _time.sleep(5)
                    elapsed += 5
                    yield ev("heartbeat", elapsed=elapsed, msg=f"Analysing clauses… ({elapsed}s)")
                t1.join()

                if "error" in _result:
                    raise Exception(_result["error"])

                stage1_result = _result.get("data", {})
                ...
                # Stage 2 — classification (also threaded with heartbeat)
                ...
                if findings:
                    ...
                    def _run_stage2():
                        try:
                            _result2["data"] = llm_json_raw(
                                system=S2,
                                prompt=f"Classify these verified findings into the summary schema:\n\n{findings_json}",
                                temperature=0.1,
                            )
                        except Exception as e:
                            _result2["error"] = str(e)

                    t2 = _threading.Thread(target=_run_stage2, daemon=True)
                    t2.start()
                    elapsed2 = 0
                    while t2.is_alive():
                        _time.sleep(5)
                        elapsed2 += 5
                        yield ev("heartbeat", elapsed=elapsed2, msg=f"Scoring risk profile… ({elapsed2}s)")
                    t2.join()

                # Build final summary (threaded)
                yield ev("progress", msg="Calculating deal score and finalising report…", step=6, total=6)
                _result3 = {}
                def _run_final():
                    try:
                        _result3["data"] = run_document_summary(documents=documents, llm_json_fn=llm_json_raw)
                    except Exception as e:
                        _result3["error"] = str(e)

                t3 = _threading.Thread(target=_run_final, daemon=True)
                t3.start()
                elapsed3 = 0
                while t3.is_alive():
                    _time.sleep(5)
                    elapsed3 += 5
                    yield ev("heartbeat", elapsed=elapsed3, msg=f"Building risk report… ({elapsed3}s)")
                t3.join()

                if "error" in _result3:
                    raise Exception(_result3["error"])
                summary = _result3.get("data", {})
