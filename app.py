...
# ============================================================
#  OPENROUTER API WRAPPER WITH SAFE LOGGING
# ============================================================

def call_openrouter(messages, model="gpt-4o-mini"):
    """Robust OpenRouter API call with retry, fallback models, and safe Supabase logging."""
    headers = {
        ...
        for attempt in range(3):  # up to 3 retries per model
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if content:
                        print(f"✅ Response successfully generated by {current_model}")
                        try:
                            log_model_event("success", current_model, attempt + 1, "Response OK")
                ...
                # --- Retry on overload ---
                    ...
                    print(f"⚠️ {current_model} overloaded (503). Retrying after delay...")
                    time.sleep(2 ** attempt)
                    continue

                # --- Unexpected status code ---
                else:
                    print(f"❌ {current_model} returned HTTP {resp.status_code}: {resp.text[:200]}")
                    try:
                        log_model_event("error", current_model, attempt + 1, f"HTTP {resp.status_code}")
                    except Exception as e:
                        print(f"⚠️ Logging skipped: {e}")
                    break

            except requests.Timeout:
                print(f"⏱ Timeout from {current_model}, retrying after delay...")
                time.sleep(2 ** attempt)
                continue
                ...
                try:
                    log_model_event("error", current_model, attempt + 1, str(e))
                except Exception as e2:
                    print(f"⚠️ Logging skipped: {e2}")
                break

        print(f"❌ All retries failed for {current_model}, moving to next model...")
    ...
    try:
        log_model_event("failed_all", "all_models", 3, "All models overloaded or unavailable")
    except Exception as e:
        print(f"⚠️ Logging skipped: {e}")
...
# ============================================================
#  SAFE SUPABASE LOGGING HELPER
# ============================================================

def log_model_event(status, model_name, attempts, message):
    """Safe background logger for Supabase model telemetry."""
    try:
        ...
        print(f"📝 Logged {status} for {model_name} after {attempts} attempts.")
    except Exception as e:
