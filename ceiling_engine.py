...
def calculate_ceiling(
    ...
    return {
        ...
        # Waterfall decomposition — fully reconciling attribution
...
def _waterfall_decomposition(
    ...
    """
    Waterfall attribution — fully reconciling decomposition.

    Method: work from base down, attributing each £ impact precisely.
    Structural and asset discounts are multiplicative; we attribute
    each component as its marginal £ impact on the running value.

    Structural step:
        after_structural = base × (1 - D_structural)
        structural_gbp   = base - after_structural

    Asset step (D_max dominant + interaction):
        after_asset = after_structural × (1 - D_asset)
        asset_gbp   = after_structural - after_asset

    Within D_asset, apportion between dominant flag and interaction:
        dominant_share = D_max / D_asset
        interaction_share = interaction_pct / D_asset

    Total check: structural_gbp + asset_gbp == base - investment_value
    This reconciles exactly.
    """
    base   = market_value
    iv     = round(base * (1 - d_total))
    total_gbp = round(base - iv)

    # Structural step
    after_struct  = base * (1 - d_structural)
    struct_gbp    = round(base - after_struct)
    struct_pct    = round(d_structural * 100, 1)

    # Asset step
    after_asset   = after_struct * (1 - d_asset)
    asset_gbp_tot = round(after_struct - after_asset)
    asset_pct     = round(d_asset * 100, 1)

    # Reconciliation rounding correction (absorb into asset)
    rounding_err  = total_gbp - struct_gbp - asset_gbp_tot
    asset_gbp_tot += rounding_err

    # Per-flag attribution within D_asset
    d_max        = asset_detail.get("d_max", d_asset)
    interaction  = d_asset - d_max
    dominant_share     = (d_max / d_asset) if d_asset > 0 else 1.0
    interaction_share  = (interaction / d_asset) if d_asset > 0 else 0.0

    dominant_gbp    = round(asset_gbp_tot * dominant_share)
    interaction_gbp = asset_gbp_tot - dominant_gbp

    # Top flag entries (drivers already sorted by impact_pct)
    top_flags = drivers[:5] if drivers else []
    flag_impacts = []
    for i, d in enumerate(top_flags):
        if i == 0:
            gbp = dominant_gbp
        else:
            # Apportion remaining interaction_gbp proportionally across secondary flags
            remaining_pct = sum(
                x["impact_pct"] for x in top_flags[1:]
            ) or 1.0
            share = (d["impact_pct"] / remaining_pct) if remaining_pct > 0 else 0
            gbp = round(interaction_gbp * share)
    ...
    primary = flag_impacts[0] if flag_impacts else None

    return {
        "base_valuation_gbp":       int(base),
        "structural_discount_pct":  struct_pct,
        "structural_discount_gbp":  struct_gbp,
        "after_structural_gbp":     round(after_struct),
        "asset_discount_pct":       asset_pct,
        "asset_discount_gbp":       asset_gbp_tot,
        "total_discount_pct":       round(d_total * 100, 1),
        "total_discount_gbp":       total_gbp,
        "investment_value_gbp":     int(iv),
        "flag_impacts":             flag_impacts,
        "primary_driver":           primary,
        # Reconciliation audit field
        "_reconciles":              (struct_gbp + asset_gbp_tot) == total_gbp,
    }
